...
(function () {
  ...
  function generateLoopEmbedCode() {
    ...
    return svgExp.buildIframeLoopEmbed(payload, { ...embedOpts, packageDir: EMBED_PACKAGE_DIR });
  }
  ...
  function embedCodeModeLabel() {
    ...
    return { mode: "package", packageDir: EMBED_PACKAGE_DIR };
  }
  ...
  function showEmbedModal() {
    ...
    try {
      ...
      if (status) {
        ...
        if (info.mode === "hosted") {
          status.textContent = `Ready to paste · player at ${info.hostUrl} · ${timing} · ${size}`;
        } else {
          status.textContent = `Ready to paste · download embed package first · ${timing} · ${size}`;
        }
      }
    } catch (err) {
      console.warn(err);
  ...
  function wireExportUi() {
    ...
    $("export-embed-btn")?.addEventListener("click", showEmbedModal);
    $("export-embed-regenerate")?.addEventListener("click", showEmbedModal);
